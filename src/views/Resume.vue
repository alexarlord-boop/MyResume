...
export default {
  ...
  methods: {
    ...
    MoveTitleParallax() {
      ...
      document.getElementById('title1').style.top = String(30 + this.GetScrolledPercentage() * 300) + "px";

      if (scrollPerc <= 0.43) {
        document.getElementById('title1').innerText = 'About';
      }
      if (scrollPerc > 0.43 && scrollPerc <= 0.80) {
        document.getElementById('title1').innerText = 'Stack';
      }
      if (scrollPerc > 0.80) {
        document.getElementById('title1').innerText = 'Links';
      }
    }
  },
  mounted() {
